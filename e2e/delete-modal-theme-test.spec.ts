...
test.describe('Delete Task Modal - Theme & Loading States', () => {
  ...
  for (const theme of themes) {
    test(`should display correctly in ${theme.name} theme`, async ({ page }) => {
      ...
      const modalBg = await modalContent.evaluate(
        (el) => window.getComputedStyle(el).backgroundColor
      );
      const modalColor = await modalContent.evaluate((el) => window.getComputedStyle(el).color);
      const modalBorder = await modalContent.evaluate(
      ...
      const warningColor = await warningBox.evaluate((el) => window.getComputedStyle(el).color);
      ...
      const deleteBtnBg = await deleteBtn.evaluate(
        (el) => window.getComputedStyle(el).backgroundColor
      );
